...
class Discriminator(nn.Module):
    def __init__(self, input_size, input_features, h2l):
        super(Discriminator, self).__init__()
        assert input_size in [16, 32, 64]
        self.blocks = [128, 128, 256, 256, 512, 512]  # 6
        pool_start = len(self.blocks) - 4
        if input_size == 32:
            pool_start = len(self.blocks) - 3
        elif input_size == 16:
            pool_start = len(self.blocks) - 2
            if not h2l:
                self.blocks = [256, 256, 256, 256, 512, 512]
...
def discriminator_test():
    in_size = 16  # 16, 32, 64
    in_feat = 3
    net = Discriminator(in_size, in_feat, True).cuda()
    X = np.random.randn(10, in_feat, in_size, in_size).astype(np.float32)
