...
parser.add_argument('-c', '--gpu', default='0',
...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    if path.endswith('.npz'):
        ...
        # for fn in files:
        # imgs = np.array([Image.open(str(fn)).convert('RGB').astype(np.float32) for fn in files])
        imgs = np.array([imread(str(fn), pilmode="RGB").astype(np.float32) for fn in files])
        # Bring images to shape (B, 3, H, W)
        imgs = imgs.transpose((0, 3, 1, 2))
        # Rescale images to be between 0 and 1
        imgs /= 255
